...
/**
 * claude-code-java 统一启动入口。
 * Unified entry point for claude-code-java.
 *
 * 默认模式 — REST API 服务器（端口 8080）：
 * Default mode — REST API server (port 8080):
 *   java -jar claude-code-java-start.jar
 *
 * CLI 交互模式 — 禁用 Web Server，进入 REPL：
 * CLI interactive mode — disables Web Server, enters REPL:
 *   java -jar claude-code-java-start.jar --spring.profiles.active=cli
 */
